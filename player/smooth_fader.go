...
func (s *smoothFader) tryStream(samples [][2]float64) (int, bool) {
	if s.stopped {
		return 0, false
	}
	...
	var n int
	var ok bool
	if ahead, okAhead := s.Streamer.(*aheadStreamer); okAhead {
		n, ok = ahead.TryStream(samples)
	} else if t, okCast := s.Streamer.(tryStreamer); okCast {
		n, ok = t.TryStream(samples)
	} else {
		n, ok = s.Streamer.Stream(samples)
	}
	if n == 0 {
...
func (s *smoothFader) gainForTrySample() float64 {
	switch {
	case s.fadingIn:
		idx := int(s.fadeIndex.Load())
		last := len(s.fadeInWindow) - 1
